...
#为了增加将RTK绝对yaw角填充到sensor_msgs::IMU类型数据中，发布到handsfree/rtk_yaw_imu话题
import math
...
class NtripGnssNode(object):
    def __init__(self):
        ...
        # 👉 新增：IMU格式yaw角发布器
        self.rtk_yaw_imu_pub = rospy.Publisher("handsfree/rtk_yaw_imu", Imu, queue_size=10)
        ...
        self.cov_by_flag = {
            ...
            # 4: 0.1, # RTK Fixed
            # 5: 1.0, # RTK Float
            4: 0.0004, # 🌟 RTK Fixed (方差 0.0004，代表标准差 2cm)
            5: 0.04,   # 🌟 RTK Float (方差 0.04，代表标准差 20cm)
    ...
    def _handle_nmea_line(self, line):
        ...
        # 结构化解析（GGA/RMC），这是得到字典类型的各种数据，通过判断字典中的'type'来识别是什么数据，进而通过不同的处理函数进行处理并发布话题
        ...
        #识别字典数据类型为GNTHS，提取字典数据中键为'headding'对应的值，利用_publish_gnths_data发布到handsfree/rtk/heading话题中（发布的是float类型数据）
        #现增加_publish_gnths_data的功能，不仅发布float类型数据到handsfree/rtk/heading中，还要发布Imu类型数据到handsfree/rtk_yaw_imu话题中
    ...
    def _publish_gngga_data(self, gga):
        try:
            ...
            # 👇 新增：记录最新的状态，供发布 Yaw 时使用
            self.current_fix_quality = fix_q
            ...
            # 从这个cov_by_flag字典中查找对应的fix_q键值对应的值赋值给cov,没有赋值给10000.0，说明解的状态fix_q和cov_by_flag中的建是一一对应的
    ...
    def _publish_gnths_data(self, ths):
        try:
            ...
            # ===================== 原有逻辑：发布Float64航向角（RTK双天线航向，这个航向是地理坐标系/航海标准：以正北（North）为 0°，顺时针（CW）旋转角度增加。因此，正东在 RTK 里是 90°，正南是 180°） =====================
            self.heading_pub.publish(Float64(data=heading))
            rospy.loginfo("THS: heading=%.3f° (valid)", heading)

            # ===================== 新增发布逻辑：将航向角转换到ROS中的ENU坐标系，以正东（East）为 0°，逆时针（CCW）旋转角度增加。因此，正北在 ROS 里是 90°（或 $\pi/2$ 弧度），正西是 180°，并发布IMU格式四元数及动态协方差 =====================
            
            # # 原来直接转换1. 角度转弧度（ROS四元数必须用弧度）
            # yaw_rad = math.radians(heading)

            #-----------start：现在加一步角度的坐标转换：地理坐标系(NED) 转 ROS(ENU) ---------------
            # 1. 应用 90 - heading 转换公式
            yaw_deg = 90.0 - heading
            
            # 【重要】天线安装偏差补偿（根据第4节的说明，如果天线不是前后安装，请在这里加减偏差）
            # yaw_deg += install_offset_deg 

            # 2. 角度归一化，确保在 [-180, 180] 之间
            ...
            # 2. 欧拉角转四元数 (roll=0, pitch=0, yaw=航向角)
            ...
            # 👇👇👇 6. 核心：动态协方差与熔断判断 👇👇👇
            cov = [0.0] * 9
            if self.current_fix_quality == 4:
                # 状态 4 (RTK Fix): 极其信任（挺信任）
                cov[8] = 0.05 
            elif self.current_fix_quality == 5:
                # 状态 5 (RTK Float): 极度降权，忍受跳变（有点信任）
                cov[8] = 0.1 
            else:
                # 状态 0,1,2 等单点或无解：绝对 Yaw 已经完全不可用！
                # 策略：直接 return 熔断，拒绝发布该话题！这是直接熔断，而不是给一个比较大的协方差，这样熔断也是可以的。
                # 从而迫使 robot_localization 触发 sensor_timeout，全面接管底层的纯 IMU 角速度航位推算！
                rospy.logwarn_throttle(1.0, "RTK Status is neither Fix nor Float. Halting absolute Yaw publishing!")
                return 

            imu_msg.orientation_covariance = tuple(cov)
            
            # 7. 发布IMU消息
            self.rtk_yaw_imu_pub.publish(imu_msg)
    ...
    # （1）这种修改适合只使用固定解的情况，我的使用条件是NavSatStatus = 2，如果不等于2我就不使用。
    # 因为包括浮点解的多个状态都被赋值为STATUS_GBAS_FIX，我如果使用STATUS_GBAS_FIX>=0的数据，就包括前面的两种，那STATUS_FIX状态的数据就需要给很大的方差。
    # 但是这样的数据正常不应该给系统使用，因为滤波器或者图优化是脆弱的。最好的就是因该只使用固定解和浮点解，所以应该把这两个状态设置为地基增强状态
    # def _ros_status_from_gga(self, flag):       
    #     if flag in (4,): # RTK Fixed
    #         return NavSatStatus.STATUS_GBAS_FIX				#如果flag = 4 ，代表是RTK固定解，就将NavSatStatus赋值为地基增强状态：STATUS_GBAS_FIX值为2
    #     if flag in (5, 2, 1, 3):                            
    #         return NavSatStatus.STATUS_FIX					#如果flag = 5, 2, 1, 3 ，代表是Float/DGPS/GPS/PPS解， 就将NavSatStatus赋值为单点定位状态：STATUS_FIX，值为0
    #     return NavSatStatus.STATUS_NO_FIX					#如果flag = 0 ，6，7，8，代表的是无效解、678不知道是什么解，就将NavSatStatus赋值为无效解状态：STATUS_NO_FIX值为-1
    
    # （2）这种修改适合即使用固定解也使用浮点解的情况，如果状态为STATUS_GBAS_FIX那我就使用，我的使用条件是NavSatStatus = 2，如果不等于2我就不使用
    def _ros_status_from_gga(self, flag):       
        # 把 4(Fix) 和 5(Float) 都归为地基增强 (STATUS_GBAS_FIX = 2)
        if flag in (4, 5): 
            return NavSatStatus.STATUS_GBAS_FIX             
        # 其他的单点定位、差分等归为普通定位 (STATUS_FIX = 0)
        if flag in (1, 2, 3): 
